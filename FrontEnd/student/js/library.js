...
async function updateLibraryView() {
    ...
    try {
        ...
        const response = await fetch(`http://127.0.0.1:8000/library/student/books?semester=${semester}`, {
            method: 'GET',
            headers: {
        ...
        books.forEach(book => {
            if (book.book_code.toLowerCase().includes(searchText) || book.title.toLowerCase().includes(searchText)) {
                
                let statusClass = "status-issued"; // Default
