document.addEventListener("DOMContentLoaded", async () => {
  ...
  try {
    ...
    const academicsRes = await fetch("http://127.0.0.1:8000/student/my-academics",{
      headers : getAuthHeaders(),
    });
    ...
    document.getElementById("p-teacher").innerText = "-"; // not provided by API

    document.getElementById("p-dob").innerText = profile.date_of_birth ?? "-";
    document.getElementById("p-gender").innerText = "-"; // not in API
    ...
    document.getElementById("p-mobile").innerText = "-"; // not in API
    document.getElementById("p-pmobile").innerText = "-"; // not in API
