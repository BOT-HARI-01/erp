...
def get_payment_details_by_roll(db, roll_no: str):
    ...
    for fee_type, total in fee_map.items():
        paid = float(payments.get(fee_type).amount_paid) if fee_type in payments else 0
...
def update_student_payment(db, req, admin_email):
    ...
    payment.amount_paid = (
        Decimal(payment.amount_paid) + Decimal(str(req.amount))
    )
    ...
    payment.status = (
        "PAID" if payment.amount_paid >= total_fee else "PENDING"
    )
...
def get_student_payment_details(db, student_email: str, semester: int):
    ...
    payments = db.query(Payment).filter(
        Payment.student_email == student_email,
    ...
    for p in payments:
        ...
        if p.amount_paid > 0:
            transactions.append({
                ...
                "remBalance": float(
                    fee_map[p.fee_type] - paid_map[p.fee_type]
                )
            })
